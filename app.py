...
def parse_dezenas_str(s: str):
    """Converte string '1, 2, 3, 4, 5, 6' ou '1 2 3 4 5 6' para set({1,2,3,4,5,6})."""
    if not s or not s.strip():
        return set()
    partes = [p for p in s.replace(",", " ").split(" ") if p.strip()]
    try:
        nums = [int(p) for p in partes]
    except:
        return set()
    # Filtra por faixa válida
...
with st.expander("⚙️ Parâmetros", expanded=True):
    ...
    # Campo manual para digitar o último resultado:
    ult_manual_str = st.text_input("Último resultado (manual) — ex.: 7, 12, 23, 31, 44, 58", "")
...
if st.button("🚀 GERAR JOGOS", use_container_width=True):
    ...
    # 1) Se checkbox marcado, tenta API
    ult = set()
    if usar_ultimo:
        ult = ler_ultimo_resultado_caixa()
        if ult:
            st.success(f"Último resultado (API CAIXA): {sorted(ult)}")
        else:
            st.warning("Não foi possível obter o último resultado via API.")

    # 2) Se você forneceu manualmente, manual sobrepõe
    ult_manual = parse_dezenas_str(ult_manual_str)
    if ult_manual:
        if len(ult_manual) == 6:
            ult = ult_manual
            st.info(f"Último resultado (manual): {sorted(ult)}")
        else:
            st.error("Último resultado (manual) deve conter exatamente 6 dezenas entre 1 e 60, sem repetição.")
            st.stop()
    else:
        # Caso nenhum válido esteja disponível, segue sem restrição de repetição
        if not ult and usar_ultimo:
            st.warning("Sem último resultado disponível; gerando sem restrição de repetição do último sorteio.")
